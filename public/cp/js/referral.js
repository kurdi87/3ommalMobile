jQuery(document).ready(function () {
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/event/listR?status=" + $('#statusN').val(),
        "order": [[0, "desc"]],
        columnDefs: [
        ...
        "columns": [
            ...
            {data: 'coverage_no', name: 'coverage_no'},
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function () {
                $(this).attr("href", $(this).data("href") + "status=" + $('#statusN').val() + "&" + input);
            });
        },
    ...
    jQuery(document).on('change', '#status', function () {
        ...
        oTable.ajax.url('crm/event/listR?status=' + this.value).load();
    });
    ...
    jQuery(document).on('click', '.statusModal', function () {
        ...
        jQuery('.modal-body-status').load('crm/event/referralStatus/' + eid);
        return false;
    });
    jQuery(document).on('submit', '#addStatus', function () {
        var thisStatus = jQuery(this);
        //  var proid = jQuery(this).find('#pro_id :selected').val();


        if (!errors) {
            jQuery.ajax({
                url: "crm/event/changeReferralStatus",

                type: 'POST',
                data: thisStatus.serialize(),
                dataType: "json",
                success: function (data) {
                    if (data.status) {
                        ...
                        jQuery('#modal-status').find('.form-control').val('');
    ...
    jQuery(document).on('click', '.emailModal', function () {
        ...
        jQuery('#' + target).modal('show');
        jQuery('.modal-body').load('crm/event/email/' + eid);
        return false;
    });
