...
jQuery(document).on('change', '.upload-headline-img', function () {
        ...
        if (flag == true) {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    url: 'tabibfind/headline/uploadImage/'+id,
                    type: 'POST',
                    data: fd,
                    cache: false,
                    contentType: false,
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "img/blog/" + data.file_name;
                            my_button.parents('.upload-headline-img').css('background-image', 'url(' + src + ')');
                            my_button.parents('.upload-headline-img').find('.icon').attr('value',data.file_name);
                        }
                        else {
                            flag = true;
