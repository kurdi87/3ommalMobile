...
@section('content')
    <div class="form-package">
        {!! Form::model($result,['action'=>[($isProfile) ? "Admin\UserController@updateProfile":"Admin\UsersController@update",$result->SysUsr_ID],'class'=>'form-validation form-datavalidation user-form', 'files' => true]) !!}
        @include('cp.users.form')
        {!! Form::close() !!}
    </div>
@stop