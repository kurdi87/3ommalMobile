...
jQuery(document).ready(function () {
   // users.init();
   var employeePayment_id= jQuery(this).find('.employeePayment_id').attr('value');
    var flag = true;
    oTable3 = $('#mydatatable3').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/employeePayment/listAtt/"+employeePayment_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('submit', '#addAtt', function () {
        ...
        if (!errors) {
            jQuery.ajax({
                url: "crm/employeePayment/addAtt",
                type: 'POST',
                ...
                success: function (data) {
                    if (data.status) {
                     ...
                     jQuery('#modal-attadd').find('.form-control').val('');
                        ...
                        jQuery('#modal-attadd').find('.upload-employeePayment-att').css('background-image', 'url( "1.jpg")');
     ...
     jQuery(document).on('click', '.attEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
          jQuery('.modal-body').load('crm/employeePayment/editAtt/'+statid);
        return false;
    });
    ...
    jQuery(document).on('submit', '#editAtt', function () {
        ...
        if (!errors) {
            jQuery.ajax({
                url: "crm/employeePayment/storeAtt",
                type: 'POST',
                data: thisAction.serialize(),
                dataType: "json",
                success: function (data) {
                    if (data.status) {
                        ...
                        jQuery('#modal-attEdit').modal('hide');
                         jQuery('#modal-attEdit').find('.upload-employeePayment-att').css('background-image', 'url(img/employeePayment/patients/1.jpg)');
...
jQuery(document).on('change', '.upload-employeePayment-att', function () {
        ...
        if (flag == true) {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    url: 'crm/employeePayment/uploadAtt/'+id,
                    type: 'POST',
                    data: fd,
                    cache: false,
                    contentType: false,
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "uploads/employeePayment/" + data.file_name;

                            my_button.parents('.upload-employeePayment-att').find('.icon').attr('value',data.file_name);
