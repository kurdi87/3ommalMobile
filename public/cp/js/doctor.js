jQuery(document).ready(function () {
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/doctor/list?hospital=" + $('#hospitalN').val() + "&department=" + $('#departmentN').val() + "&speciality=" + $('#specialityN').val()+ "&city=" + $('#cityN').val(),
        "order": [[1, "desc"]],
        columnDefs: [
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function () {
                $(this).attr("href", $(this).data("href") + "hospital=" + $('#hospitalN').val() + "&department=" + $('#departmentN').val() + "&speciality=" + $('#specialityN').val()+ "&city=" + $('#cityN').val(), +input);
            });
        },
    ...
    jQuery(document).on('click', '.btn-submit-search', function () {
        oTable.ajax.url("crm/doctor/list?hospital=" + $('#hospitalN').val() + "&department=" + $('#departmentN').val() + "&speciality=" + $('#specialityN').val()+ "&city=" + $('#cityN').val()).load();
    });
    ...
    jQuery(document).on('change', '#speciality', function () {
        var status = $('#specialityN').attr('value', this.value);
        oTable.ajax.url("crm/doctor/list?hospital=" + $('#hospitalN').val() + "&department=" + $('#departmentN').val() + "&speciality=" + $('#specialityN').val()+ "&city=" + $('#cityN').val()).load();
    });
    jQuery(document).on('change', '#city', function () {
        var status = $('#cityN').attr('value', this.value);
        oTable.ajax.url("crm/doctor/list?hospital=" + $('#hospitalN').val() + "&department=" + $('#departmentN').val() + "&speciality=" + $('#specialityN').val()+ "&city=" + $('#cityN').val()).load();    });
