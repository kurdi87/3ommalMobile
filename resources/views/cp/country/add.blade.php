...
@section('js')
    ...
    <script src="cp/js/usersForm.js" type="text/javascript"></script>
        <script src="cp/js/countryForm.js" type="text/javascript"></script>

     
    @if($errors->has())
        <script>
            jQuery(document).ready(function () {
                  toasterMessage('error', 'Errors number : {{ $errors->first() }}', 'Please ceheck errors');
            });
...
@section('content')
    <div class="form-package">
        {!! Form::open(['action'=>'Admin\CountryController@store','class'=>'form-validation form-datavalidation']) !!}
        @include('cp.country.form')
        {!! Form::close() !!}
    </div>
@stop