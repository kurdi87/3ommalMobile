...
jQuery(document).ready(function () {
   // users.init();
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/exception/list?status="+"&hospital="+$('#hospitalN').val()+"&finance_party="+$('#finance_partyN').val()+"&department="+$('#departmentN').val()+"&employee="+$('#employeeN').val()+"&from="+$('#from').val()+"&to="+$('#to').val(),
        "order": [[0, "desc"]],
        columnDefs: [
       ...
       {orderable: false, targets: 1},
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function(){
                $(this).attr("href", $(this).data("href") +"status="+$('#statusN').val()+"&hospital="+$('#hospitalN').val()+"&department="+$('#departmentN').val()+"&employee="+$('#employeeN').val()+"&agent="+$('#agentN').val()+"&finance_party="+$('#finance_party').val()+"&from="+$('#from').val()+"&to="+$('#to').val(),+input);
            });
        },
    ...
    jQuery(document).on('click', '.btn-reset', function () {
        ...
        $('.filter').attr('value',0);
        oTable.ajax.url("crm/exception/list?status="+$('#statusN').val()+"&agent="+$('#agentN').val()+"&hospital="+$('#hospitalN').val()+"&finance_party="+$('#finance_partyN').val()+"&department="+$('#departmentN').val()+"&employee="+$('#employeeN').val()+"&from="+$('#from').val()+"&to="+$('#to').val()).load();
    });
    ...
    jQuery(document).on('click', '.processModal', function () {
        ...
        jQuery('.modal-body').load('crm/exception/process/'+eid);
        return false;
    });
    jQuery(document).on('click', 'body', function (e) {
        var target = $(e.target);
    ...
    jQuery(document).on('click', '.btn-mstatus', function () {
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                oTable.ajax.url("crm/exception/list?status="+$('#statusN').val()+"&agent="+$('#agentN').val()+"&hospital="+$('#hospitalN').val()+"&employee="+$('#employeeN').val()+"&finance_party="+$('#finance_partyN').val()+"&department="+$('#departmentN').val()+"&from="+$('#from').val()+"&to="+$('#to').val()).load();
            }
        });
