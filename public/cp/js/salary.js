jQuery(document).ready(function () {
    ...
    oTable = $('#mydatatable').DataTable({
        ...
        "ajax": "crm/salary/list?status=" + $('#statusN').val() + "&city=" + $('#cityN').val()+ "&user_id=" + $('#user_id').val()+"&from="+$('#from').val()+"&to="+$('#to').val(),
        "order": [[0, "desc"]],
        columnDefs: [
    ...
    jQuery(document).on('change', '#status', function () {
        ...
        oTable.ajax.url("crm/salary/list?status="  + $('#statusN').val() + "&finance_party=" + $('#finance_partyN').val()+ "&hospital=" + $('#hospitalN').val()+ "&city=" + $('#cityN').val()+ "&employee=" + encodeURI($('#employeeN').val())+ "&region=" + $('#regionN').val()+ "&uploaded=" + $('#uploadedN').val()+"&from="+$('#from').val()+"&to="+$('#to').val()+ "&type=" + $('#typeN').val()).load();
    });
    ...
    jQuery(document).on('change', '#from', function () {
        ...
        oTable.ajax.url("crm/salary/list?status="+ $('#statusN').val() + "&city=" + $('#cityN').val()+ "&user_id=" + $('#user_id').val()+"&from="+$('#from').val()+"&to="+$('#to').val()).load();
    });
