...
jQuery(document).on('change', '.upload-financeParty-img', function () {
        ...
        if (flag == true) {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    url: 'crm/financeParty/uploadImage/'+id,
                    type: 'POST',
                    data: fd,
                    cache: false,
                    contentType: false,
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "cp/images/" + data.file_name;
                            my_button.parents('.upload-financeParty-img').css('background-image', 'url(' + src + ')');
                            my_button.parents('.upload-financeParty-img').find('.icon').attr('value',data.file_name);
                        }
                        else {
                            flag = true;
