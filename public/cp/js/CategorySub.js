...
jQuery(document).ready(function () {
    // users.init();
    var category_id= jQuery(this).find('.category_id').attr('value');
    var flag = true;
    oTable1 = $('#mydatatable1').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/category/listSubCategorys/"+category_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('click', '.subEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#div1').load('crm/category/editSub/'+subid);
        jQuery('#' + target).modal('show');
        return false;
    });
