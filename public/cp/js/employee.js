...
jQuery(document).ready(function () {
   // users.init();
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/employee/list?type="+$('#typeN').val()+"&department="+$('#departmentN').val()+"&manager="+$('#managerN').val(),
        "order": [[ 2, "desc" ]],
        columnDefs: [
       ...
       //{orderable: false, targets: 0},
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function(){
                $(this).attr("href",$(this).data("href") +"type="+$('#typeN').val()+"&department="+$('#departmentN').val()+"&manager="+$('#managerN').val()+input);
            });
        },
    ...
    jQuery(document).on('click', '.btn-reset', function () {
        ...
        $('.filter').attr('value',0);
        oTable.ajax.url("crm/employee/list?type="+$('#typeN').val()+"&department="+$('#departmentN').val()+"&manager="+$('#managerN').val()).load();
    });
    ...
    jQuery(document).on('change', '#manager', function () {
        var status=$('#managerN').attr('value',this.value);
        oTable.ajax.url("crm/employee/list?type="+$('#typeN').val()+"&department="+$('#departmentN').val()+"&manager="+$('#managerN').val()).load();
    });
