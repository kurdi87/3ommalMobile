...
jQuery(document).on('change', '.upload-doctor-img', function () {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "img/team/" + data.file_name;
                            my_button.parents('.upload-doctor-img').css('background-image', 'url(' + src + ')');
                            my_button.parents('.upload-doctor-img').find('.icon').attr('value',data.file_name);
