jQuery(document).ready(function () {
    ...
    oTable = $('#mydatatable').DataTable({
        ...
        "ajax": "crm/job/list?status=" + $('#statusN').val() + "&city=" + $('#cityN').val()+"&category=" + encodeURI($('#categoryN').val())+ "&user_id=" + $('#user_id').val()+"&from="+$('#from').val()+"&to="+$('#to').val(),
        "order": [[0, "desc"]],
        columnDefs: [
        ...
        "columns": [
            ...
            {data: 'name', name: 'name'},
            {data: 'sid', name: 'sid'},
            {data: 'work_fields', name: 'work_fields'},
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function () {
                $(this).attr("href", $(this).data("href") + $('#statusN').val() + "&city=" + $('#cityN').val()+"&category=" + encodeURI($('#categoryN').val())+ "&user_id=" + $('#user_id').val()+"&from="+$('#from').val()+"&to="+$('#to').val() + input);
            });
        },
    ...
    jQuery(document).on('change', '#status', function () {
        ...
        oTable.ajax.url("crm/job/list?status="  + $('#statusN').val() + "&finance_party=" + $('#finance_partyN').val()+ "&hospital=" + $('#hospitalN').val()+ "&city=" + $('#cityN').val()+ "&employee=" + encodeURI($('#employeeN').val())+ "&region=" + $('#regionN').val()+ "&uploaded=" + $('#uploadedN').val()+"&from="+$('#from').val()+"&to="+$('#to').val()+ "&type=" + $('#typeN').val()).load();
    });
    ...
    jQuery(document).on('change', '#from', function () {
        ...
        oTable.ajax.url("crm/job/list?status="+ $('#statusN').val() + "&city=" + $('#cityN').val()+"&category=" + encodeURI($('#categoryN').val())+ "&user_id=" + $('#user_id').val()+"&from="+$('#from').val()+"&to="+$('#to').val()).load();
    });
    ...
    jQuery(document).on('change', '#category', function () {
        var action = $('#categoryN').attr('value', this.value);
        oTable.ajax.url("crm/job/list?status="+ $('#statusN').val() + "&city=" + $('#cityN').val()+"&category=" + encodeURI($('#categoryN').val())+ "&user_id=" + $('#user_id').val()+"&from="+$('#from').val()+"&to="+$('#to').val()).load();
    });

    jQuery(document).on('change', '#user_id', function () {
        var action = $('#user_id').attr('value', this.value);
        oTable.ajax.url(("crm/job/list?status="+ $('#statusN').val() + "&city=" + $('#cityN').val()+"&category=" + encodeURI($('#categoryN').val())+ "&user_id=" + $('#user_id').val()+"&from="+$('#from').val()+"&to="+$('#to').val())).load();
    });
    ...
    jQuery(document).on('click', '.btn-mstatus', function () {
        ...
        if (thisclick.hasClass('mstatus-inactive'))
            ...
            status = "1";
        if (confirm("This will delete message...Are you sure?")) {
            jQuery.ajax({
