jQuery(document).ready(function () {
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/payment/list?type=" + $('#typeN').val() + "&commission=" + $('#commissionN').val() + "&month=" + $('#monthN').val() + "&year=" + $('#yearN').val()+ "&employee=" + $('#employeeN').val()+ "&service_provider=" + $('#service_providerN').val(),
        "order": [[0, "desc"]],
        columnDefs: [
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function () {
                $(this).attr("href", $(this).data("href") + "type=" + $('#typeN').val() + "&commission=" + $('#commissionN').val() + "&month=" + $('#monthN').val() + "&year=" + $('#yearN').val()+ "&employee=" + $('#employeeN').val()+ "&service_provider=" + $('#service_providerN').val() + "&" + input);
            });
        },
    ...
    jQuery(document).on('click', '.btn-submit-search', function () {
        oTable.ajax.url("crm/payment/list?type=" + $('#typeN').val() + "&commission=" + $('#commissionN').val() + "&month=" + $('#monthN').val() + "&year=" + $('#yearN').val()+ "&employee=" + $('#employeeN').val()+ "&service_provider=" + $('#service_providerN').val()).load();
    });
    ...
    jQuery(document).on('change', '#commissionN', function () {
        var action = $('#commissionN').attr('value', this.value);
        oTable.ajax.url("crm/payment/list?type=" + $('#typeN').val() + "&commission=" + $('#commissionN').val() + "&month=" + $('#monthN').val() + "&year=" + $('#yearN').val()+ "&employee=" + $('#employeeN').val()+ "&service_provider=" + $('#service_providerN').val()).load();
    });
    jQuery(document).on('change', '#service_providerN', function () {
        var action = $('#service_providerN').attr('value', this.value);
        oTable.ajax.url("crm/payment/list?type=" + $('#typeN').val() + "&service_provider=" + $('#service_providerN').val() + "&month=" + $('#monthN').val() + "&year=" + $('#yearN').val()+ "&employee=" + $('#employeeN').val()+ "&commission=" + $('#commissionN').val()).load();
    });
    ...
    jQuery(document).on('change', '#status', function () {
        ...
        oTable.ajax.url('crm/payment/list?status=' + this.value).load();
    });
    ...
    jQuery(document).on('click', '.processModal', function () {
        ...
        jQuery('.modal-body').load('crm/payment/process/' + eid);
        return false;
    });
    jQuery(document).on('click', 'body', function (e) {
        var target = $(e.target);
    ...
    jQuery(document).on('click', '.cstatusModal', function () {
        ...
        jQuery('#' + target).modal('show');
        jQuery('.modal-body-cstatus').load('crm/payment/cstatus/' + eid);
        return false;
    });
