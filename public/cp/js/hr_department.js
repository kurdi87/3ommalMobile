...
jQuery(document).ready(function () {
   // users.init();
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/hr_department/list?&department="+$('#departmentN').val()+"&manager="+$('#managerN').val(),
        "order": [[ 0, "desc" ]],
        columnDefs: [
       ...
       //{orderable: false, targets: 0},
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function(){
                $(this).attr("href",$(this).data("href") +"&department="+$('#departmentN').val()+"&manager="+$('#managerN').val()+input);
            });
        },
    ...
    jQuery(document).on('click', '.btn-reset', function () {
        ...
        $('.filter').attr('value',0);
        oTable.ajax.url("crm/hr_department/list?&department="+$('#departmentN').val()+"&manager="+$('#managerN').val()).load();
    });
