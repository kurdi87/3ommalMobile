...
jQuery(document).on('change', '.upload-patient-img', function () {
        ...
        if (flag == true) {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    url: 'crm/patient/uploadImage/'+id,
                    type: 'POST',
                    data: fd,
                    cache: false,
                    contentType: false,
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "img/patient/" + data.file_name;
                            my_button.parents('.upload-patient-img').css('background-image', 'url(' + src + ')');
                            my_button.parents('.upload-patient-img').find('.icon').attr('value',data.file_name);
                        }
                        else {
                            flag = true;
