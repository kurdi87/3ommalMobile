...
@section('content')
    ...
    <div class="site-container">
        <form class="form ajaxForm myform" id="myform" action="/{{ config('app.user_route_name')}}/updateProfile"
              method="post">
            <div class="form-group">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <label>الاسم رباعي</label>
                <input type="text" name="SysUsr_FullName" class="form-control" value="{{$user->SysUsr_FullName}}"
            ...
            <div class="form-group">
                ...
                <div class="multi-input">
                    <div class="ck-button">
                        <label>
                            <input type="radio" name="sid_type" {{$user->sid_type==1?'checked':''}} value="1"/><span>فلسطينية</span>
                        </label>
                    </div>
                    <div class="ck-button">
                        <label>
                            <input type="radio" name="sid_type" {{$user->sid_type==2?'checked':''}} value="2"/><span>اسرائيلية</span>
                        </label>
                    </div>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="input-with-icon">
                    <input name="SysUsr_DoB"
                           type="date"
                           value="{{$user->SysUsr_DoB?Date('Y-m-d',strtotime($user->SysUsr_DoB)):''}}"
            ...
            <div class="form-group">
                ...
                <div class="multi-input">
                    <div class="ck-button">
                        <label>
                            <input type="radio" {{$user->gender=='m'?'checked':''}} name="gender"
                                   value="m"/><span>ذكر</span>
                        </label>
                    </div>
                    <div class="ck-button">
                        <label>
                            <input type="radio" {{$user->gender=='f'?'checked':''}} name="gender"
            ...
            <div class="form-group">
                ...
                <div class="multi-input">
                    <div class="ck-button">
                        <label>
                            <input type="radio" {{$user->social_status==1?'checked':''}} name="social_status"
                                   value="1"/><span>اعزب</span>
                        </label>
                    </div>
                    <div class="ck-button">
                        <label>
                            <input type="radio" {{$user->social_status==2?'checked':''}} name="social_status"
            ...
            <div class="form-group">
                ...
                <div class="custom-select">
                    <select name="city">
                        ...
                        @foreach($city as $c)
                            <option value="{{$c->id}}" {{$c->id==$user->city?'selected':''}}> {{$c->name_ar}} </option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="upload-btn-wrapper">
                    ...
                    <input type="file" att_id="{{$user->SysUsr_ID}}" upload="sid_attach" class="upload-request"
                           link="/{{ config('app.user_route_name')}}/uploadFile?module=sid_attach&title=sid&att_id={{$user->SysUsr_ID}}"
                    name="sid_attach"/>
                </div>
                ...
                <div class="text-center">
                    ...
                    <a href="/uploads/{{isset($sid_attach)?$sid_attach->name:''}}" target="_blank" id="sid_attach" class="upload {{isset($sid_attach)?'':'hidden'}} rft2 text-left"
            ...
            <div class="form-group">
                ...
                <div class="upload-btn-wrapper">
                    ...
                    <input type="file" att_id="{{$user->SysUsr_ID}}"  upload="card_attach" name="card_attach" class="upload-request"
                           link="/{{ config('app.user_route_name')}}/uploadFile?module=card_attach&title=card&att_id={{$user->SysUsr_ID}}"/>
                </div>
                <div class="text-center">
                            <span class="uploading hidden" data-spinner="1" style=""><img src="/img/load.gif"
                                                                                          style="height: 60px;width: auto;margin: auto;position: relative;display: block;"></span>
                    <a href="/uploads/{{isset($sid_attach)?$sid_attach->name:''}}" target="_blank" id="card_attach" class="upload {{isset($sid_attach)?'':'hidden'}} rft2 text-left"
                       style=""><span>تم رفع الملف بنجاح إضغط هنا<i
                                    class="glyphicon glyphicon-check"></i></span> </a>
                </div>
            </div>
