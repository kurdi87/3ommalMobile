...
let yearName = "year_work",
    monthName = "month_work",
    yes = "نعم",
    no = "لا",
    ...
    $(".steps li").each(function (index) {
        ...
        if (title === toQuestion) {
            if (questionId) {
                ...
                if (!canPass) {
                    showMessage('الرجاء التأكد من القيمة المدخلة');
                    return;
                }
            }
...
function questionValidation(questionId, extraValidation) {
    ...
    if(extraValidation) {
        switch(extraValidation) {
            case "salary":
                if(getVal< 100 || getVal> 10000) valid = false; break;
            case "years":
                if(getVal< 1 || getVal> 30) valid = false;break;
        }
    }
    return valid;
}
