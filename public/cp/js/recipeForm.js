...
jQuery(document).on('change', '.upload-recipe-img', function () {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "img/recipe/" + data.file_name;
                            my_button.parents('.upload-recipe-img').css('background-image', 'url(' + src + ')');
                            my_button.parents('.upload-recipe-img').find('.icon').attr('value',data.file_name);
