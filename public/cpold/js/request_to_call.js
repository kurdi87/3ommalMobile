...
jQuery(document).ready(function () {
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "hitechjobs/request_to_call/list?status="+$('#statusN').val(),
        "order": [[ 0, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('change', '#status', function () {
        ...
        oTable.ajax.url('hitechjobs/request_to_call/list?status='+this.value).load();
    });
    ...
    jQuery(document).on('click', '.processModal', function () {
        ...
        jQuery('.modal-body').load('hitechjobs/request_to_call/process/'+eid);
        return false;
    });
    jQuery(document).on('click', 'body', function (e) {
        var target = $(e.target);
