...
jQuery(document).on('change', '.upload-event-photo', function () {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "img/event/gallery/" + data.file_name;
                            my_button.parents('.upload-event-photo').css('background-image', 'url(' + src + ')');
                            my_button.parents('.upload-event-photo').find('.icon').attr('value',data.file_name);
                        }
                        else {
                            flag = true;
