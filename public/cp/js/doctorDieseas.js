...
jQuery(document).ready(function () {
   ...
   var doctor_id= jQuery(this).find('.doctor_id').attr('value');
    ...
    oTable6 = $('#mydatatable6').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/doctor/listDieseas/"+doctor_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('submit', '#addDieseas', function () {
        var thisAction = jQuery(this);
         var dieseasid = jQuery(this).find('#dieseas_id :selected').val();

        if (!errors) {
            jQuery.ajax({
                url: "crm/doctor/addDieseas",
                type: 'POST',
                data: thisAction.serialize(),
                dataType: "json",
                success: function (data) {
                    if (data.status) {
                        ...
                        $("#dieseas_id option[value="+dieseasid+"]").remove();
    ...
    jQuery(document).on('click', '.dieseasEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
          jQuery('.modal-body').load('crm/doctor/editDieseas/'+dieseasid);
        return false;
    });
