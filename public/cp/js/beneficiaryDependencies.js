...
jQuery(document).ready(function () {
   ...
   var beneficiary_id= jQuery(this).find('.beneficiary_id').attr('value');
    ...
    oTable4 = $('#mydatatable4').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/beneficiary/listDependencies/"+beneficiary_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('submit', '#addDependencies', function () {
        ...
        if (!errors) {
            jQuery.ajax({
                url: "crm/beneficiary/addDependencies",
                type: 'POST',
     ...
     jQuery(document).on('click', '.dependenciesEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
          jQuery('.modal-body').load('crm/beneficiary/editDependencies/'+statid);
        return false;
    });
    ...
    jQuery(document).on('submit', '#editDependencies', function () {
        ...
        if (!errors) {
            jQuery.ajax({
                url: "crm/beneficiary/storeDependencies",
                type: 'POST',
                data: thisAction.serialize(),
                dataType: "json",
                success: function (data) {
...
jQuery(document).on('change', '.upload-beneficiary-dependencies', function () {
        ...
        if (flag == true) {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    url: 'crm/beneficiary/uploadDependencies/'+id,
                    type: 'POST',
                    data: fd,
                    cache: false,
                    contentType: false,
