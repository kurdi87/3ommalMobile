...
jQuery(document).ready(function () {
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/request_to_call/list?status="+$('#statusN').val(),
        "order": [[ 0, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('change', '#status', function () {
        ...
        oTable.ajax.url('crm/request_to_call/list?status='+this.value).load();
    });
    ...
    jQuery(document).on('click', '.processModal', function () {
        ...
        jQuery('.modal-body').load('crm/request_to_call/process/'+eid);
        return false;
    });
    jQuery(document).on('click', 'body', function (e) {
        var target = $(e.target);
