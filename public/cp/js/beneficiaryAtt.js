...
jQuery(document).ready(function () {
    ...
    oTable3 = $('#mydatatable3').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/beneficiary/listAtt/"+beneficiary_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('click', '.attEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
        jQuery('.modal-body').load('crm/beneficiary/editAtt/'+statid);
        return false;
    });
    ...
    jQuery(document).on('change', '.upload-beneficiary-att', function () {
        ...
        if (flag == true) {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    url: 'crm/beneficiary/uploadAtt/'+id,
                    type: 'POST',
                    data: fd,
                    cache: false,
                    contentType: false,
