...
jQuery(document).ready(function () {
   // users.init();
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/commitment/list?status="+$('#statusN').val()+"&recipe="+$('#hospitalN').val()+"&department="+$('#departmentN').val()+"&attached="+$('#attachedN').val()+"&region="+$('#regionN').val()+"&procedure="+$('#procedureN').val()+"&finance_party="+$('#finance_partyN').val()+ "&from=" + $('#from').val()+ "&to=" + $('#to').val(),
        "order": [[ 0, "desc" ]],
        columnDefs: [
       ...
       //{orderable: false, targets: 0},
        ...
        "columns": [
            ...
            {data: 'est_amount', name: 'est_amount'},
            {data: 'service_date', name: 'service_date'},
            {data: 'Procedures_codes', name: 'Procedures_codes'},
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function(){
                $(this).attr("href", $(this).data("href") +"status="+$('#statusN').val()+"&recipe="+$('#hospitalN').val()+"&department="+$('#departmentN').val()+"&attached="+$('#attachedN').val()+"&region="+$('#regionN').val()+"&procedure="+$('#procedureN').val()+"&finance_party="+$('#finance_partyN').val()+ "&from=" + $('#from').val()+ "&to=" + $('#to').val()+"&"+ input);
            });
        },
    ...
    jQuery(document).on('change', '#status', function () {
        ...
        oTable.ajax.url("crm/commitment/list?status="+$('#statusN').val()+"&recipe="+$('#hospitalN').val()+"&department="+$('#departmentN').val()+"&attached="+$('#attachedN').val()+"&region="+$('#regionN').val()+"&procedure="+$('#procedureN').val()+"&finance_party="+$('#finance_partyN').val()+ "&from=" + $('#from').val()+ "&to=" + $('#to').val()).load();
    });
    ...
    jQuery(document).on('change', '#procedure', function () {
        var action=$('#procedureN').attr('value',this.value);
        oTable.ajax.url("crm/commitment/list?status="+$('#statusN').val()+"&recipe="+$('#hospitalN').val()+"&department="+$('#departmentN').val()+"&attached="+$('#attachedN').val()+"&region="+$('#regionN').val()+"&procedure="+$('#procedureN').val()+"&finance_party="+$('#finance_partyN').val()+ "&from=" + $('#from').val()+ "&to=" + $('#to').val()).load();
    });
    ...
    jQuery(document).on('change', '#attached', function () {
        var action=$('#attachedN').attr('value',this.value);
        oTable.ajax.url("crm/commitment/list?status="+$('#statusN').val()+"&recipe="+$('#hospitalN').val()+"&department="+$('#departmentN').val()+"&attached="+$('#attachedN').val()+"&region="+$('#regionN').val()+"&procedure="+$('#procedureN').val()+"&finance_party="+$('#finance_partyN').val()+ "&from=" + $('#from').val()+ "&to=" + $('#to').val()).load();
    });
    ...
    jQuery(document).on('click', '.emailModal', function () {
        ...
        jQuery('.modal-body').load('crm/commitment/email/' + eid);
        return false;
    });
    ...
    jQuery(document).on('click', '.btn-reset', function () {
        $('.form-control').val('');
        oTable.ajax.url("crm/commitment/list?status="+$('#statusN').val()+"&action="+$('#actionN').val()+"&from="+$('#from').val()+"&to="+$('#to').val()).load();
    });
    ...
    jQuery(document).on('change', '#status', function () {
        var status=$('#statusN').attr('value',this.value);
        oTable.ajax.url('crm/commitment/list?status='+this.value).load();
    });
    ...
    jQuery(document).on('click', '.processModal', function () {
        ...
        jQuery('#'+ target).modal('show');
        jQuery('.modal-body').load('crm/commitment/process/'+eid);
        return false;
    });
    jQuery(document).on('click', 'body', function (e) {
        var target = $(e.target);
    ...
    jQuery(document).on('click', '.cstatusModal', function () {
        ...
        jQuery('#'+ target).modal('show');
        jQuery('.modal-body-cstatus').load('crm/commitment/cstatus/'+eid);
        return false;
    });
