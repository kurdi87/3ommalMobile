...
jQuery(document).ready(function () {
    ...
    oTable1 = $('#mydatatable1').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/appointment/listProcedure/"+appointment_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('click', '.proEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
        jQuery('.modal-body').load('crm/appointment/editPro/'+proid);
        return false;
    });
