jQuery(document).ready(function () {
    ...
    oTable1 = $('#mydatatable1').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/commitment/listProcedure/" + commitment_id,
        "order": [[1, "desc"]],
        columnDefs: [
        ...
        "footerCallback": function (row, data, start, end, display) {
            ...
            $(api.column(4).footer()).html(
                '<input class="totalcostpro form-control " name="tt" type="text" value="'+total+'">'
            );
            jQuery(document).find('.totalcostpro').change();
        }
    ...
    jQuery(document).on('click', '.proEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
        jQuery('.modal-body').load('crm/commitment/editPro/' + proid);
        return false;
    });
