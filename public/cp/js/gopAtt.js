   ...
   var gop_id= jQuery(this).find('.gop_id').attr('value');
    ...
    jQuery(document).on('submit', '#addAtt', function () {
                ...
                success: function (data) {
                    if (data.status) {
                     ...
                     jQuery('#modal-attadd').find('.form-control').val('');
                        ...
                        jQuery('#modal-attadd').find('.upload-gop-att').css('background-image', 'url( "1.jpg")');
     ...
     jQuery(document).on('click', '.attEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
          jQuery('.modal-body').load('crm/gop/editAtt/'+statid);
        return false;
    });
                ...
                success: function (data) {
                    if (data.status) {
                        ...
                        jQuery('#modal-attEdit').modal('hide');
                         jQuery('#modal-attEdit').find('.upload-gop-att').css('background-image', 'url(img/gop/patients/1.jpg)');
...
jQuery(document).on('change', '.upload-gop-att', function () {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "uploads/gop/" + data.file_name;

                            my_button.parents('.upload-gop-att').find('.icon').attr('value',data.file_name);
