...
jQuery(document).ready(function () {
   // users.init();
   var appointment_id= jQuery(this).find('.appointment_id').attr('value');
    var flag = true;
    oTable3 = $('#mydatatable3').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/appointment/listAtt/"+appointment_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('submit', '#addAtt', function () {
                ...
                success: function (data) {
                    if (data.status) {
                     ...
                     jQuery('#modal-attadd').find('.form-control').val('');
                        ...
                        jQuery('#modal-attadd').find('.upload-appointment-att').css('background-image', 'url( "1.jpg")');
     ...
     jQuery(document).on('click', '.attEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
          jQuery('.modal-body').load('crm/appointment/editAtt/'+statid);
        return false;
    });
                ...
                success: function (data) {
                    if (data.status) {
                        ...
                        jQuery('#modal-attEdit').modal('hide');
                         jQuery('#modal-attEdit').find('.upload-appointment-att').css('background-image', 'url(img/appointment/patients/1.jpg)');
...
jQuery(document).on('change', '.upload-appointment-att', function () {
        ...
        if (flag == true) {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    url: 'crm/appointment/uploadAtt/'+id,
                    type: 'POST',
                    data: fd,
                    cache: false,
                    contentType: false,
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "uploads/appointment/" + data.file_name;

                            my_button.parents('.upload-appointment-att').find('.icon').attr('value',data.file_name);
