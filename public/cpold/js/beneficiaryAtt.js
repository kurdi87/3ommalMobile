...
jQuery(document).ready(function () {
    ...
    oTable3 = $('#mydatatable3').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "hitechjobs/beneficiary/listAtt/"+beneficiary_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('submit', '#addAtt', function () {
        ...
        if (!errors) {
            jQuery.ajax({
                url: "hitechjobs/beneficiary/addAtt",
                type: 'POST',
                ...
                success: function (data) {
                    if (data.status) {
                        ...
                        jQuery('#modal-attadd').find('.upload-beneficiary-att').css('background-image', 'url( "1.jpg")');
    ...
    jQuery(document).on('click', '.attEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
        jQuery('.modal-body').load('hitechjobs/beneficiary/editAtt/'+statid);
        return false;
    });
    ...
    jQuery(document).on('submit', '#editAtt', function () {
        ...
        if (!errors) {
            jQuery.ajax({
                url: "hitechjobs/beneficiary/storeAtt",
                type: 'POST',
                data: thisAction.serialize(),
                dataType: "json",
                success: function (data) {
                    if (data.status) {
                        ...
                        jQuery('#modal-attEdit').find('.upload-beneficiary-att').css('background-image', 'url(img/beneficiary/beneficiarys/1.jpg)');
    ...
    jQuery(document).on('change', '.upload-beneficiary-att', function () {
        ...
        if (flag == true) {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    url: 'hitechjobs/beneficiary/uploadAtt/'+id,
                    type: 'POST',
                    data: fd,
                    cache: false,
                    contentType: false,
                    ...
                    success: function (data) {
                        ...
                        if (data.status == true) {
                            ...
                            my_button.parents('.upload-beneficiary-att').find('.icon').attr('value',data.file_name);
