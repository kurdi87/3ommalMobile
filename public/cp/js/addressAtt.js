...
jQuery(document).ready(function () {
   ...
   var address_id= jQuery(this).find('.address_id').attr('value');
    ...
    oTable3 = $('#mydatatable3').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/address/listAtt/"+address_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('submit', '#addAtt', function () {
     ...
     jQuery(document).on('click', '.attEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
          jQuery('.modal-body').load('crm/address/editAtt/'+statid);
        return false;
    });
