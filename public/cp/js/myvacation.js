jQuery(document).ready(function () {
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "ecareer/vacation/myList?status=" + $('#status').val() + "&from=" + $('#from').val() + "&to=" + $('#to').val() + "&type=" + $('#type').val() + "&department=" + $('#department').val() + "&employee=" + $('#employee').val(),
        "order": [[0, "desc"]],
        columnDefs: [
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function () {
                $(this).attr("href", $(this).data("href") + "status=" + $('#status').val() + "&action=" + $('#action').val() + "&from=" + $('#from').val() + "&to=" + $('#to').val() + "&hospital=" + $('#hospital').val() + "&doctor=" + $('#doctor').val() + "&department=" + $('#department').val() + "&employee=" + $('#employee').val() + "&finance_party=" + $('#finance_party').val(), +"&" + input);
            });
        },
    ...
    jQuery(document).on('change', '#status', function () {
        ...
        oTable.ajax.url("ecareer/vacation/myList?status=" + $('#status').val() + "&from=" + $('#from').val() + "&to=" + $('#to').val() + "&type=" + $('#type').val() + "&department=" + $('#department').val() + "&employee=" + $('#employee').val()).load();
    });
    ...
    jQuery(document).on('click', '.visitModal', function () {
        ...
        jQuery('.modal-body-visit').load('ecareer/vacation/nextvacation/' + eid);
        return false;
    });
    jQuery(document).on('submit', '#addVisit', function () {
        ...
        if (!errors) {
            jQuery.ajax({
                url: "ecareer/vacation/addnextvacation",
    ...
    jQuery(document).on('click', '.actionModal', function () {
        ...
        jQuery('#' + target).modal('show');
        jQuery('.modal-body-actio').load('ecareer/vacation/actionvacation/' + eid);
        return false;
    });
    jQuery(document).on('submit', '#addAction', function () {
        ...
        if (!errors) {
            jQuery.ajax({
                url: "ecareer/vacation/addactionvacation",
    ...
    jQuery(document).on('click', '.btn-reset', function () {
        $('.form-control').val('');
        oTable.ajax.url("ecareer/vacation/myList?status=" + $('#status').val() + "&action=" + $('#action').val() + "&from=" + $('#from').val() + "&to=" + $('#to').val() + "&hospital=" + $('#hospital').val() + "&doctor=" + $('#doctor').val() + "&department=" + $('#department').val() + "&employee=" + $('#employee').val()).load();
    });
    ...
    jQuery(document).on('change', '#status', function () {
        var status = $('#status').attr('value', this.value);
        oTable.ajax.url('ecareer/vacation/myList?status=' + this.value).load();
    });
    ...
    jQuery(document).on('click', '.processModal', function () {
        ...
        jQuery('#' + target).modal('show');
        jQuery('.modal-body').load('ecareer/vacation/process/' + eid);
        return false;
    });
    jQuery(document).on('click', 'body', function (e) {
        var target = $(e.target);
    ...
    jQuery(document).on('click', '.reminder', function () {
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                oTable.ajax.url("ecareer/vacation/myList?status=" + $('#status').val() + "&action=" + $('#action').val() + "&from=" + $('#from').val() + "&to=" + $('#to').val()).load();
    ...
    jQuery(document).on('click', '.emailModal', function () {
        ...
        jQuery('#' + target).modal('show');
        jQuery('.modal-body').load('ecareer/vacation/email/' + eid);
        return false;
    });
