...
jQuery(document).ready(function () {
   // users.init();
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/event/list?status="+"&recipe="+$('#hospitalN').val()+"&finance_party="+$('#finance_partyN').val()+"&department="+$('#departmentN').val()+"&employee="+$('#employeeN').val()+"&from="+$('#from').val()+"&to="+$('#to').val(),
        "order": [[0, "desc"]],
        columnDefs: [
       ...
       {orderable: false, targets: 1},
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function(){
                $(this).attr("href", $(this).data("href") +"status="+$('#statusN').val()+"&recipe="+$('#hospitalN').val()+"&department="+$('#departmentN').val()+"&employee="+$('#employeeN').val()+"&agent="+$('#agentN').val()+"&finance_party="+$('#finance_party').val()+"&from="+$('#from').val()+"&to="+$('#to').val(),+input);
            });
        },
    ...
    jQuery(document).on('click', '.btn-reset', function () {
        ...
        $('.filter').attr('value',0);
        oTable.ajax.url("crm/event/list?status="+$('#statusN').val()+"&agent="+$('#agentN').val()+"&recipe="+$('#hospitalN').val()+"&finance_party="+$('#finance_partyN').val()+"&department="+$('#departmentN').val()+"&employee="+$('#employeeN').val()+"&from="+$('#from').val()+"&to="+$('#to').val()).load();
    });
    ...
    jQuery(document).on('click', '.btn-mstatus', function () {
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                oTable.ajax.url("crm/event/list?status="+$('#statusN').val()+"&agent="+$('#agentN').val()+"&recipe="+$('#hospitalN').val()+"&employee="+$('#employeeN').val()+"&finance_party="+$('#finance_partyN').val()+"&department="+$('#departmentN').val()+"&from="+$('#from').val()+"&to="+$('#to').val()).load();
            }
        });
