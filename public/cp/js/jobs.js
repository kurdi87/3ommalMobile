...
jQuery(document).ready(function () {
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/jobs/list?status="+$('#status').val() + "&type=" + $('#type').val()+ "&active=" + $('#active').val(),
        "order": [[ 0, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('change', '#status', function () {
        ...
        oTable.ajax.url('crm/jobs/list?status='+this.value+ "&type=" + $('#type').val()+ "&active=" + $('#active').val()).load();
    });
    ...
    jQuery(document).on('click', '.processModal', function () {
        ...
        jQuery('.modal-body').load('crm/jobs/process/'+eid);
        return false;
    });
    jQuery(document).on('click', 'body', function (e) {
        var target = $(e.target);
