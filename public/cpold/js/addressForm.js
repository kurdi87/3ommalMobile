...
jQuery(document).on('change', '.upload-address-img', function () {
        ...
        if (flag == true) {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    url: 'hitechjobs/address/uploadImage/'+id,
                    type: 'POST',
                    data: fd,
                    cache: false,
                    contentType: false,
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "img/address/" + data.file_name;
                            my_button.parents('.upload-address-img').css('background-image', 'url(' + src + ')');
                            my_button.parents('.upload-address-img').find('.icon').attr('value',data.file_name);
                        }
                        else {
                            flag = true;
