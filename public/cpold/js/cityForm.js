...
jQuery(document).on('change', '.upload-city-img', function () {
        ...
        if (flag == true) {
            ...
            if (size <= 8096 && type == true) {
                ...
                jQuery.ajax({
                    url: 'hitechjobs/city/uploadImage/'+id,
                    type: 'POST',
                    data: fd,
                    cache: false,
                    contentType: false,
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "img/city/" + data.file_name;
                            jQuery('.form-body').find('.uploading').addClass('hidden');
                            my_button.parents('.upload-city-img').css('background-image', 'url(' + src + ')');
                            my_button.parents('.upload-city-img').find('.icon').attr('value',data.file_name);
                        }
                        else {
                            flag = true;
