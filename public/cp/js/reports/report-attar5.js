...
var reportAttar = function () {
    ...
    var handleAjaxReport = function() {
        var no_articles = jQuery('.noarticles-select').val();
        var date_from = jQuery('.txt-inputfrom').val();
        var date_to = jQuery('.txt-inputto').val();
        var article_types_select = jQuery('.articletype-select').val();
        //console.log(package_types_select);
        var article_types = [];
        if(article_types_select != null)
        {
            article_types = article_types_select;
        }
        jQuery.ajax({
            type: 'GET',
            url: 'http://dcetest.com/new_attar/public/cp-attar/topInteractionArticles',
            data: {
                articlesNo: no_articles,
                'types[]': article_types,
                from: date_from,
                to: date_to
            },
            ...
            success: function (data) {
                ...
                for (var i = 0; i < result.length; i++) {
                    xaxies.push(result[i].ArtLang_ArticleTitle);
                    yaxies.push(parseFloat(result[i].commentsNo));
                    serial.push(parseFloat(result[i].likesNo));
                }
                //console.log(result);
                if(result.length)
                {
                ...
                var oTable = $('#mysample_1,.mysample_1').DataTable({
                    ...
                    "aoColumns": [
                        { "mDataProp": "serial" },
                        { "mDataProp": "ArtLang_ArticleTitle" },
                        { "mDataProp": "type" },
                        { "mDataProp": "commentsNo" },
                        { "mDataProp": "likesNo" },
                    ],
                    buttons: [
    ...
    var handleChangeElements = function() {
        ...
        jQuery('.noarticles-select').change(function() {
        ...
        jQuery('.articletype-select').change(function() {
            var thisclick = jQuery(this);
            var val = thisclick.val();
    ...
    var handleChart_one = function(xaxies, yaxies, serial) {
        ...
        jQuery('select.informationshown-select').change(function() {
            var thisclick = jQuery(this);
            if(thisclick.val() == "commentsNo")
            {
                $('#report_chart_column_1').highcharts(options2);
            }
            else if(thisclick.val() == "likesNo")
            {
                $('#report_chart_column_1').highcharts(options3);
            }
            else
