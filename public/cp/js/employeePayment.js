jQuery(document).ready(function () {
    ...
    oTable = $('#mydatatable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/employeePayment/list?type=" + $('#typeN').val() + "&project=" + $('#projectN').val() + "&month=" + $('#monthN').val() + "&year=" + $('#yearN').val()+ "&employee=" + $('#employeeN').val(),
        "order": [[0, "desc"]],
        columnDefs: [
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $(".exportData").each(function () {
                $(this).attr("href", $(this).data("href") + "type=" + $('#typeN').val() + "&project=" + $('#projectN').val() + "&month=" + $('#monthN').val() + "&year=" + $('#yearN').val()+ "&employee=" + $('#employeeN').val() + "&" + input);
            });
        },
    ...
    jQuery(document).on('click', '.btn-submit-search', function () {
        oTable.ajax.url("crm/employeePayment/list?type=" + $('#typeN').val() + "&project=" + $('#projectN').val() + "&month=" + $('#monthN').val() + "&year=" + $('#yearN').val()+ "&employee=" + $('#employeeN').val()).load();
    });
    ...
    jQuery(document).on('change', '#projectN', function () {
        var action = $('#projectN').attr('value', this.value);
        oTable.ajax.url("crm/employeePayment/list?type=" + $('#typeN').val() + "&project=" + $('#projectN').val() + "&month=" + $('#monthN').val() + "&year=" + $('#yearN').val()+ "&employee=" + $('#employeeN').val()).load();
    });
    ...
    jQuery(document).on('change', '#status', function () {
        ...
        oTable.ajax.url('crm/employeePayment/list?status=' + this.value).load();
    });
    ...
    jQuery(document).on('click', '.processModal', function () {
        ...
        jQuery('.modal-body').load('crm/employeePayment/process/' + eid);
        return false;
    });
    jQuery(document).on('click', 'body', function (e) {
        var target = $(e.target);
    ...
    jQuery(document).on('click', '.cstatusModal', function () {
        ...
        jQuery('#' + target).modal('show');
        jQuery('.modal-body-cstatus').load('crm/employeePayment/cstatus/' + eid);
        return false;
    });
    jQuery(document).on('submit', '#addCstatus', function () {
        ...
        if (!errors) {
            jQuery.ajax({
                url: "crm/employeePayment/addcstatus",
