...
jQuery(document).ready(function () {
   // users.init();
   var patient_id= jQuery(this).find('.patient_id').attr('value');
    var flag = true;
    oTable3 = $('#mydatatable3').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/patient/listAtt/"+patient_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('submit', '#addAtt', function () {
                ...
                success: function (data) {
                    if (data.status) {
                     ...
                     jQuery('#modal-attadd').find('.form-control').val('');
                        ...
                        jQuery('#modal-attadd').find('.upload-patient-att').css('background-image', 'url( "1.jpg")');
     ...
     jQuery(document).on('click', '.attEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
          jQuery('.modal-body').load('crm/patient/editAtt/'+statid);
        return false;
    });
                ...
                success: function (data) {
                    if (data.status) {
                        ...
                        jQuery('#modal-attEdit').modal('hide');
                         jQuery('#modal-attEdit').find('.upload-patient-att').css('background-image', 'url(img/patient/patients/1.jpg)');
