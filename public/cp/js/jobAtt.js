...
jQuery(document).ready(function () {
   // users.init();
   var job_id= jQuery(this).find('.job_id').attr('value');
    var flag = true;
    oTable3 = $('#mydatatable3').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "crm/job/listAtt/"+job_id,
        "order": [[ 1, "desc" ]],
        columnDefs: [
    ...
    jQuery(document).on('submit', '#addAtt', function () {
                ...
                success: function (data) {
                    if (data.status) {
                     ...
                     jQuery('#modal-attadd').find('.form-control').val('');
                        ...
                        jQuery('#modal-attadd').find('.upload-job-att').css('background-image', 'url( "1.jpg")');
     ...
     jQuery(document).on('click', '.attEditmodal', function () {
        ...
        var target = jQuery(this).attr('data-modal');
        jQuery('#' + target).modal('show');
          jQuery('.modal-body').load('crm/job/editAtt/'+statid);
        return false;
    });
                ...
                success: function (data) {
                    if (data.status) {
                        ...
                        jQuery('#modal-attEdit').modal('hide');
                         jQuery('#modal-attEdit').find('.upload-job-att').css('background-image', 'url(img/job/patients/1.jpg)');
...
jQuery(document).on('change', '.upload-job-att', function () {
            ...
            if (size <= 4096 && type == true) {
                ...
                jQuery.ajax({
                    ...
                    //    my_button.parent().find('.loading-submit').remove();
                        if (data.status == true) {
                            var src = "uploads/job/" + data.file_name;

                            my_button.parents('.upload-job-att').find('.icon').attr('value',data.file_name);
